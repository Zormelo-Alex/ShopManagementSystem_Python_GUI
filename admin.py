...
def valid_phone(phn):
    if re.match(r"[025]\d{9}$", phn):
        return True
    return False

def valid_aadhar(phn2):
    if phn2.isdigit() and len(phn2)==10:
        return True
    return False
...
class login_page:
    def __init__(self, top=None):
        top.geometry("1366x750")
        top.resizable(0, 0)
        top.title("Manager(ADMINISTRATOR)")

        self.label1 = Label(root)
        self.label1.place(relx=0, rely=0, width=1366, height=750)
...
class Employee:
    def __init__(self, top=None):
        ...
        self.tree.heading("Aadhar No.", text="Other No.", anchor=W)
